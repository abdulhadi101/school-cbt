            ...
            <nav class="border-b border-gray-100 bg-white">
                        ...
                        <div class="flex">
                            ...
                            <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                                ...
                                <template v-if="isStaff">
                                    <NavLink v-for="link in staffLinks" :key="link.href" :href="link.href" :active="link.active">
                                        {{ link.label }}
                                    </NavLink>
                                </template>
                                <template v-if="isStudent && !isStaff">
                                    <NavLink v-for="link in studentLinks" :key="link.href" :href="link.href" :active="link.active">
                                        {{ link.label }}
                                    </NavLink>
                                </template>
                            </div>
                ...
                <div :class="{ block: showingNavigationDropdown, hidden: !showingNavigationDropdown }" class="sm:hidden">
                    <div class="space-y-1 pb-3 pt-2">
                        ...
                        <template v-if="isStaff">
                            <ResponsiveNavLink v-for="link in staffLinks" :key="link.href" :href="link.href" :active="link.active">
                                {{ link.label }}
                            </ResponsiveNavLink>
                        </template>
                        <template v-if="isStudent && !isStaff">
                            <ResponsiveNavLink v-for="link in studentLinks" :key="link.href" :href="link.href" :active="link.active">
                                {{ link.label }}
                            </ResponsiveNavLink>
                        </template>
                    </div>
