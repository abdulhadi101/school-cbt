...
const submitPreview = () => {
    ...
    previewForm.post(route('staff.users.staff.preview'));
};

const submitImport = () => {
    if (! previewFile.value) return;
    importForm.csv_file = previewFile.value;
    importForm.post(route('staff.users.staff.import'));
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-semibold leading-tight text-gray-800">Import Staff</h2>
                    <p class="text-sm text-gray-500">Upload a CSV file to bulk-create staff accounts.</p>
                </div>
            ...
            <div class="mx-auto max-w-5xl sm:px-6 lg:px-8 space-y-6">
                <div class="rounded-xl bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-4 flex gap-3">
                        <a :href="route('staff.users.staff.template')" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                            Download Template
                        </a>
                        <a :href="route('staff.users.staff.export')" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    ...
                    <div class="mt-4 rounded-lg bg-slate-50 p-4 text-xs text-slate-600">
                        ...
                        <p class="mt-1"><code>name</code>, <code>email</code>, <code>password</code> (optional, defaults to password12345), <code>roles</code> (comma-separated slugs), <code>first_name</code>, <code>last_name</code>, <code>middle_name</code>, <code>staff_number</code>, <code>can_publish_exams</code> (yes/no)</p>
                        <p class="mt-2">Available role slugs: <span v-for="(role, i) in roles" :key="role.id"><code>{{ role.name }}</code><span v-if="i < roles.length - 1">, </span></span></p>
                    </div>
                </div>
                ...
                <div v-if="importResult" class="rounded-xl bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-4 space-y-2">
                        <p class="text-sm text-green-700">{{ importResult.created }} staff imported successfully.</p>
                    ...
                    <div class="mt-4">
                        <Link :href="route('staff.users.staff.index')" class="text-sm text-slate-600 hover:text-slate-900">Go to Staff List</Link>
                    </div>
                </div>
